...
data "azurerm_virtual_network" "vnet_applz" {
  ...
  name                = "vnet-xl-stg-eastus-001"
  resource_group_name = "rg-network-xl-eastus-002"
}

data "azurerm_subnet" "snet_pe" {
  provider             = azurerm.applz
  name                 = "snet-xl-stg-pe-001"
  virtual_network_name = "vnet-xl-stg-eastus-001"
  resource_group_name  = "rg-network-xl-eastus-002"
}
