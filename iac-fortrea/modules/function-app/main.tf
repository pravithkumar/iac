...
resource "azurerm_role_assignment" "function_app_role" {
  ...
  role_definition_name = "Storage Blob Data Contributor"                               # Or use role_definition_id
  principal_id         = azurerm_linux_function_app.fa.identity[0].principal_id             # Replace with the principal's object ID
}